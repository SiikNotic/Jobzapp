...
export async function createQuote(jobId: string, formData: FormData): Promise<QuoteActionResult> {
...
export async function convertQuoteToInvoice(jobId: string, quoteId: string): Promise<InvoiceActionResult> {
  const { supabase, companyId, userId } = await requireOwnerCompany();
  if (!companyId) return { success: false, error: "forbidden" };
